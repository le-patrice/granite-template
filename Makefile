...
.PHONY: down
down: ## Stop and remove all containers and network mesh cleanly
	@echo -e "$(YELLOW)Stopping stack containers...$(NC)"
	@$(COMPOSE_BASE) stop -t 5 2>/dev/null || true
	@$(COMPOSE_BASE) down || ( \
		echo -e "$(YELLOW)Handling rootless namespace cleanup...$(NC)"; \
		podman unshare -- killall -9 slirp4netns pasta 2>/dev/null || true; \
		podman rm -f postgres_db 2>/dev/null || true; \
		$(COMPOSE_BASE) down \
	)
	@echo -e "$(GREEN)✅ Containers stopped cleanly$(NC)"

.PHONY: down-volumes
down-volumes: ## Stop stack and remove all persistent data volumes (CAUTION)
	@echo -e "$(RED)⚠️  Deleting all data volumes including database data. Ctrl+C to cancel...$(NC)"
	@sleep 3
	@$(COMPOSE_BASE) stop -t 5 2>/dev/null || true
	@$(COMPOSE_BASE) down -v || ( \
		echo -e "$(YELLOW)Handling rootless namespace cleanup...$(NC)"; \
