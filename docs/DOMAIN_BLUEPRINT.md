...
```mermaid
flowchart TD
    subgraph Edge_Security ["Perimeter Security (Layers 5 & 11)"]
        CF[Cloudflare Zero Trust Tunnel / Edge CDN] -->|Trusted CIDRs| TR[Traefik v3 Reverse Proxy Gateway]
        TR -->|Rate Limit: 150 req/m, 50 burst| GRANIAN[Granian Rust ASGI Server]
    end
    ...
    subgraph Data_Storage ["Persistence, CDC & Distributed Tasks (Layers 3, 6, 7 & 10)"]
        PROTOCOLS -->|PgBouncer Safe RLS| PG[(PostgreSQL 16 Relational DB)]
...
| :--- | :--- | :--- |
| **Layer 1: Frontend SPA** | Modern dashboard UI with 1:1 design tokens and full light/dark theme switching | React 18, Vite 6, Tailwind CSS v3, Lucide Icons |
| **Layer 2: API Gateway** | HTTP routing, typed dependency injection, explicit OpenAPI operations | Litestar 2.10+, Granian ASGI (4 workers), `msgspec` DTOs |
...
*Authored by Principal Enterprise Systems Architecture Team — Version 3.0.0 (2026)*
